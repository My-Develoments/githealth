...
type GovernanceDataState = "loading" | "ready" | "empty" | "error";

function resolveDataState(state: GitHubHealthIntegrationState): GovernanceDataState {
  if (state === "loading") {
    return "loading";
  }
...
function governanceScoreTone(score: number): "healthy" | "warning" | "critical" | "unknown" {
  if (score >= 85) {
    return "healthy";
  }
        ...
        <section className="gvs-grid" aria-label="Governance content">
          <Panel tone="elevated" className="gvs-card gvs-summary cc-reveal cc-reveal--hero">
            <div className="gvs-card__header">
              ...
              <Badge tone={governanceScoreTone(governanceSignal?.score ?? 0)}>
              ...
              <ul className="gvs-repo-list" aria-label="Repository governance context">
                {governanceRiskRepositories.map((repository) => (
                  <li key={repository.id}>
                    ...
                    <Badge tone={governanceScoreTone(repository.governanceScore)}>{repository.governanceScore}</Badge>
                  </li>
                ))}
              </ul>
            )}
